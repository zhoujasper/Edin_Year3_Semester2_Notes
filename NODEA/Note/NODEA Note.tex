...
\textbf{Special Case|Euler Method}: For $\frac{dy}{dt}=By$, the Euler method is $y_{n+1}=(I+hB)y_n$. where $\lambda_i$ is eigenvalues of $B$. \quad Assume $f(y)=\lambda y$
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
...
\textbf{Corollary}: For $\frac{dy}{dt}=By$ with $B$ diagonalizable. \ \ An RK Method with \textit{stability function} $R(\mu)$ is \textit{stable}|\textit{asy.stab}|\textit{unstable} at \textit{origin} iff: \qquad Assume $f(y)=\lambda_i y$
...
\textbf{Stability Region}: {\small For $\frac{dy}{dt}=By$. An RK method, the \textit{stability region} is the set of $\mu$ where $\widehat{R}(\mu)=|R(\mu)|<1$.} {\tiny ($f(y)=\lambda y$,如$y$是向量,$R(\mu)$按上面corollary的remark所说)} 

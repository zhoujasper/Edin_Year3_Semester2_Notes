...
\hspace{235pt} $^2$ only one multiplicity root of $\rho(\zeta)=0$ has modulus $|\zeta|=1$. \quad {\tiny (重根! e.g. 1,1 算,但-1,1 不算)}
...
\textbf{Stability Function}: {\small We can write the \textit{stability function} as: $R(z)=\frac{P(z)}{Q(z)}$, where $P(z)$ and $Q(z)$ are polynomials.} \quad {\scriptsize 来证明symmetric stability region的.}

\textbf{Dahlquist Test Equation}: $y'=\lambda y$ with $\lambda\in\mathbb{C}$.

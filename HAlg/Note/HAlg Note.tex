...
\np\textbf{Criteria Lemma}: If $U$, $W$ are subspace of $V$, then $V=U\oplus W$ $\Leftrightarrow$ $V=U+W$ and $U\cap W=\{0\}$. {\scriptsize (需要证明)}
...
\section{Inner Product Spaces | Orthogonal Complement / Proj} % Inner Product Spaces | Orthogonal Complement / Proj
...
\quad \textbf{Remark}: For $\{\vec{v}_1,...,\vec{v}_n\}$ orthonormal basis. \quad $\vec{v}=\sum_{i=1}^n(\vec{v},\vec{v}_i)\vec{v}_i$. \textbf{Theorem}: Every \underline{finite}-dimensional inner product space has an orthonormal basis.

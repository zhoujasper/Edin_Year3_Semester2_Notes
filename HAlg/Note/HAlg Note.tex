...
\textbf{Relate to Exponential}: If $A=D+N$, $D$ diagonalisable, $N$ nilpotent, $DN=ND$. \quad Then: $e^A=e^De^N=P^{-1}\text{diag}(e^{\lambda_1},...,e^{\lambda_n})Pe^{N}$ and $e^{At}=P^{-1}\text{diag}(e^{t\lambda_1},...,e^{t\lambda_n})Pe^{tN}$

\textbf{For Triangularisable}: 

...
\textbf{Triangularisability}: Let $f:V\to V$ be End. $V$ is finite-dimensional. the following are equivalent:
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
...
\quad \textbf{Application}: {\scriptsize 矩阵$A$换基到上三角矩阵,通过找$A$特征值及其对应的特征向量,组成新的基(可能需要广义特征向量/或拓展基),然后计算$A$在新基下的矩阵表示.}

\textbf{Diagonalisable}: Let $f:V\to V$ be End, $V$ is \textit{diagonalisable} iff $\exists$ basis of $V$ consisting of eigenvectors of $f$.

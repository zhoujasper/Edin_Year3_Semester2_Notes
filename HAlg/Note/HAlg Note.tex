...
\textbf{Group Homomorphism}: $G,H$ be groups, $\phi:G\to H$ is a homomorphism if: $\forall g_1,g_2\in G$, $\phi(g_1*g_2)=\phi(g_1)*\phi(g_2)$

\quad {\scriptsize \textbf{I}: $\phi(e_G)=e_H$; \quad \textbf{II}: $\phi(g^{-1})=\phi(g)^{-1}$ \quad \textbf{III}: Both $im \phi,Ker\phi$ are subgroups. \quad \textbf{IV}: If $K\subseteq G$ is a subgroup, then $\phi(K)\subseteq H$ is a subgroup. \quad \textbf{V}: $\phi$ is injective $\Leftrightarrow$ $Ker\phi=\{e_G\}$}

\textbf{Normal $(H\lhd G)$}: {\small $H\subseteq G$ is normal if: $\forall g\in G$, $gH=Hg$} \qquad \quad {\scriptsize \textbf{I}: $Ker\phi\lhd G$ \quad \textbf{II}: $\phi$ is 1-1 $\Rightarrow$ $G\cong im\phi$}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item \textbf{Unit}: If $^1f(1_R)=1_S$, $^2x$ is \textit{unit} of $R$. \ \ $\Rightarrow$ \ \ $^1f(x)$ is \textit{unit} of $S$. \quad $^2f|_{R^\times}:R^\times\to S^\times$ is \textit{group homomorphism}.
\end{enumerate}

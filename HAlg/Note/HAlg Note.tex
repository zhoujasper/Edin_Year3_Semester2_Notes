...
\textbf{External Direct Sum}: {\footnotesize 一个"代数结构"(Vector Space), 定义为set是$V_1\oplus\cdots\oplus V_n:=V_1\times\cdots\times V_n$且有一组运算法则component-wise operations}
...
\fbox{\parbox[c]{0.98\textwidth}{
\ex\textbf{Division Ring}: {\footnotesize If $R$ is ring and it's multiplicative inverse $\forall a\in R\setminus\{0_R\}$} \qquad \ex\textbf{Characteristic of Field}: {\footnotesize $F$ is field, $char(F)=p$ if 可以有$1/p$的操作.}
}}
...
\qquad \textbf{Property}: If $I,J$ are \textit{ideals} of $R$. Then \ \ $I+J$ \ ; \ $I\cap J$ \ \ are also ideals. \quad || \quad {\tiny Property of $m\mathbb{Z}$: $a\mathbb{Z}+b\mathbb{Z}=\gcd(a,b)\mathbb{Z}; \ \ a\mathbb{Z}\cap b\mathbb{Z}=\text{lcm}(a,b)\mathbb{Z}$ \quad Not Principal Ideal: $_{\mathbb{Z}[X]}\langle2,X\rangle$}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textbf{GS|LI}: $|L|\leq|E|$ {\scriptsize (can get: $\dim$ unique)} \quad \textbf{LI$\to$Basis}: If $V$ finite generate $\Rightarrow$ $\forall L$ can extend to a basis. {\scriptsize If $L=\emptyset$, prove $\exists B$} \quad $ker f\cap im f=\{0\}$
    ...
    \item \textbf{Dimension Theorem}: If $U,W\subseteq V$ are subspaces of $V$, then $\dim(U+W)=\dim U+\dim W-\dim(U\cap W)$ \qquad {\tiny $\downarrow$特别同构于$F^n$}
...
\textbf{Criteria Lemma}: If $U$, $W$ are subspace of $V$, then $V=U\oplus W$ $\Leftrightarrow$ $V=U+W$ and $U\cap W=\{0\}$. {\scriptsize (需要证明/只能两个向量空间)}
}}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textcolor{red}{\textbf{Determined}: $f$ is determined by $f(\vec{b_i}),\vec{b_i}\in \mathcal{B}_{basis}$ {\footnotesize (\star \ \textbf{i.e.} $f(\sum_{i}\lambda_i\vec{v_i}):=\sum_{i}\lambda_if(\vec{v_i})$)}}
...
\textbf{Matrix}: For $A_{n\times m},B_{m\times p}$, \textcolor{red}{$AB_{n\times p}:=(AB)_{ij}=\sum_{k=1}^{m}a_{ik}b_{kj}$} \quad \textbf{Transpose}: $A^T_{m\times n}:=(A^T)_{ij}=a_{ji}$
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item $_{\mathcal{B}}[f]_{\mathcal{A}}:=A$ {\small (matrix)} where \textcolor{red}{$f(\vec{v}_{i})=\sum_{j}A_{ji}\vec{w}_j$} \quad \quad $\exists M^{\mathcal{A}}_{\mathcal{B}}:Hom_{F}(V,W)\bij Mat(n\times m;F)$
    \item If $\vec{v}\in V$, then $_{\mathcal{A}}[\vec{v}]:=\mathbf{b}$ {\small (vector)} where $\vec{v}=\sum_{i}b_i\vec{v_i}$
    \item \textcolor{red}{\textbf{Theorems}}: $[f\circ g]=[f]\circ[g]$ \quad \quad $_{\mathcal{C}}[f\circ g]_{\mathcal{A}}=_{\mathcal{C}}[f]_{\mathcal{B}}\circ_{\mathcal{B}}[g]_{\mathcal{A}}$ \quad \quad $_{\mathcal{B}}[f(\vec{v})]=_{\mathcal{B}}[f]_{\mathcal{A}}\circ_{\mathcal{A}}[\vec{v}]$ \quad \quad $_{\mathcal{A}}[f]_{\mathcal{A}}=I\Leftrightarrow f=id$
    \item \textcolor{red}{\textbf{Change of Basis}}: {\small Define \textit{Change of Basis Matrix}$:=$$_{\mathcal{A}}[id_V]_{\mathcal{B}}$ \quad \quad {\footnotesize $_{\mathcal{B}'}[f]_{\mathcal{A}'}=_{\mathcal{B}'}[id_W]_{\mathcal{B}}\circ$$_{\mathcal{B}}[f]_{\mathcal{A}}\circ$$_{\mathcal{A}}[id_V]_{\mathcal{A}'}$ \quad \quad $_{\mathcal{A}'}[f]_{\mathcal{A'}}=_{\mathcal{A}}[id_V]_{\mathcal{A'}}^{-1}\circ$$_{\mathcal{A}}[f]_{\mathcal{A}}\circ$$_{\mathcal{A}}[id_V]_{\mathcal{A}'}$}}
\end{enumerate}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item {\small Set of \textit{End} is in a bijection correspondence with the equivalence class of matrices under $\sim$.} {\scriptsize \textbf{一个自同态End就对应一个相似矩阵的等价类}}
\end{enumerate}
}}
...
\fbox{\parbox[c]{0.98\textwidth}{
\textbf{Integral Domain}: A \textit{commutative} ring $R$ is an integral domain if it has no zero-divisors. \qquad \quad {\scriptsize e.g. $\mathbb{Z}/p\mathbb{Z},\mathbb{R},\mathbb{Q},\mathbb{Z},...$} \qquad {\scriptsize Cancellation Law: $\downarrow$}

\textbf{Properties of Integral Domain}: $\forall a,b\in R$. \quad \textbf{I}. $ab=0$ $\Rightarrow$ $a=0$ or $b=0$ \quad \textbf{II}. $a,b\ne0$ $\Rightarrow$ $ab\ne0$ \qquad \textbf{III}. $ab=ac,a\ne0$ $\Rightarrow$ $b=c$
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item \textbf{Lemma}: {\small $^1$ $R$ \textcolor{red}{\textit{integral domain|no zero-divisors}} $\Rightarrow$ $R[X]$ also. \ \ and \ \ $\deg(PQ)=\deg(P)+\deg(Q)$ if $P,Q\ne0$.}
    \item \textcolor{red}{\textbf{Division and Remainder}}: If $R$ is \textit{integral domain} and $P,Q\in R[X]$,{\scriptsize $Q$ \textbf{monic}} $\exists!$ $A,B\in R[X]$ s.t. $P=AQ+B$ and $\deg(B)<\deg(Q)$
...
\textbf{Equivalence Relation}: {\small \textbf{Reflexive}: $xRx$ {\scriptsize ($x\sim x$)} \ \textbf{Symmetric}: $xRy\Rightarrow y Rx$ {\scriptsize ($x\sim y \Rightarrow y\sim x$)} \ \textbf{Transitive}: $xRy,yRz\Rightarrow xRz$ {\scriptsize ($x\sim y,y\sim z\Rightarrow x\sim z$)}}

\textbf{Partial Order}: {\small \textbf{Reflexive}: $xRx$ {\scriptsize ($x\sim x$)} \ \textbf{Anti-symmetric}: $xRy,yRx\Rightarrow x = y$ {\scriptsize ($x\sim y,y\sim x\Rightarrow x=y$)} \ \textbf{Transitive}: $xRy,yRz\Rightarrow xRz$ {\scriptsize ($x\sim y,y\sim z\Rightarrow x\sim z$)}}
...
\fbox{\parbox[c]{0.98\textwidth}{
\textbf{Coset of Ideal}: Let $I$ be an ideal of $R$. Then $a+I$ is a coset of $I$. \quad \textcolor{red}{The $\sim$ is defined by $a\sim b$ $\Leftrightarrow$ $a-b\in I$ is an equivalence relation.}
}}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item \star The \underline{Identity} of $R/I$: $1_R+I$ \quad The \underline{Zero} of $R/I$: $0_R+I$
\end{enumerate}
}}
...
\textbf{Create Modules}: Let $f:R\to S$ is a \textit{ring homomorphism}, $f(1_R)=1_S$ and $M$ is a $S-$Module, then $M$ is also a $R-$Module by:
...
\quad ps: {\scriptsize $\uparrow$ consider $f:N\to (N+K)/K$ and then we can find $ker(f)=N\cap K$ \qquad $\downarrow$ consider $f:M/K\to M/N$ and then we can find $ker(f)=N/K$}
...
\np\textbf{For Vector Space}: $\dim(V/U)=\dim V-\dim U$ {\scriptsize Hint: Consider $can:V\to V/U$ by $v\mapsto v+U$ is surjection, $\ker(can)=U$}
...
\textbf{Review of Permutation}: A bijection $\sigma:\{1,...,n\}\bij\{1,...,n\}$ is a permutation. \quad All permutations of $n$ elements form a group $\mathfrak{S}_n$.

\quad {\scriptsize \textbf{Transposition}: 交换两个元素. \qquad \textbf{Inversion}: A pair of elements $(i,j)$ is an inversion of $\sigma\in\mathfrak{S}_n$ if $i<j$ but $\sigma(i)>\sigma(j)$ \qquad $\text{sgn}(a_1a_2)=-1$ \qquad $\text{sgn}(a_1...a_n)=(-1)^{n-1}$}

\quad {\scriptsize $\text{sgn}(\sigma\tau)=\text{sgn}(\sigma)\text{sgn}(\tau)$ \qquad \textbf{Alternating Group}: $A_n:=\{\sigma\in\mathfrak{S}_n:\text{sgn}(\sigma)=1\}$ \qquad \underline{\textbf{Graph Meaning of Inversion}}: Inversion is \# 画出的图中,线段交叉的次数}
...
\quad \textbf{Example}: If $H(\vec{u},\vec{v})=\vec{u}^TA\vec{v}$ $\Rightarrow$ multilinear form. \quad If $A^T=A$, symmetric. \quad If $A^T=-A$, antisymmetric.

\textbf{Property of Determinant}: Let $A,B$ be $n\times n$ matrices. \quad $F$ be field. \quad $R$ be \textit{commutative ring}.
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textbf{Unique on Field}: $\det:F^n\times \cdots\times F^n\to F$ \textit{or} $\det: Mat(n;F) \to F$ is the \textcolor{red}{$^1$\textit{unique} $^2$\textit{alternating} $^3$\textit{multilinear form} s.t. $\det(I_n)=1_F$}
    ...
    \item \textcolor{red}{\textbf{Similar on Field}}: For $F$ field. \quad $A\sim B$ $\Rightarrow$ $\det(A)=det(P^{-1}BP)=\det(B)$ \qquad Thus, we can define: $det(f)$ for $f:V\to V$
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item {\small \textbf{Adjugate Matrix}: In $R$ \ \ $\text{adj}(A)$ matrix, $\text{adj}(A)_{ij}:=C_{ji}$ \quad \textcolor{red}{\textbf{Cramer's Rule}: In $R$ \ \ $A\cdot\text{adj}(A)=(\det A)I_n$} \quad In $F$, $x_i=\frac{\det(A_i)}{\det(A)}$ {\tiny $A_i$代表$A$的第$i$列替换为$b$}}
    ...
    \item \textcolor{red}{\textbf{Invertibility of Matrix}}: In $R$, matrix $A$ is invertible $\Leftrightarrow$ $\det(A)\in R^\times$ \qquad {\scriptsize e.g. $\mathbb{Z}^\times=\{\pm 1\} \ \ ; \ \ \mathbb{C}^\times,\mathbb{R}^\times,\mathbb{Q}^\times=\mathbb{C}^*,\mathbb{R}^*,\mathbb{Q}^* \ \ ; \ \ \mathbb{F}_p^\times=\mathbb{F}_p\setminus\{0\} \ \ ; \ \ \mathbb{Z}[i]=\{\pm1,\pm i\}$}
    \item \textcolor{red}{\textbf{Jacobi's Formula}}, Let matrix $A$ s.t. $a_{ij}(t)$ are functions of $t$. \quad Then, $\frac{d}{dt}\det(A)=\text{tr}\left(\text{adj}A\cdot\frac{dA}{dt}\right)$
\end{enumerate}
...
\textbf{Existence of Eigenvalues}: For all $f:V\to V$ linear map. \ \ $^1V$ is finite-dimensional. \ \ \underline{$^1F$ is \textit{algebraically closed}}. \quad $\Rightarrow$ \quad $\exists$ eigenvalues.
...
\quad \textcolor{red}{\textbf{Similar Matrix}}: If $R$ is \textit{commutative ring}, \ $A,B\in Mat(n;R)$ similar. \quad $\Rightarrow$ \quad \textcolor{red}{$\chi_A(x)=\chi_B(x)$} \qquad Thus: \textcolor{red}{$\chi_f(x):=\chi_{_{\mathcal{A}}[f]_{\mathcal{A}}}(x)$}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item The characteristic polynomial \textcolor{red}{$\chi_f(x)$ can be factored into \underline{linear factors} over $F$.} \qquad {\scriptsize (ps: If $F$ is algebraically closed, then $f$ is triangularisable)}
\end{enumerate}

\quad \textbf{Corollary I}: Let $A,B \in Mat(n;F)$. \quad \textcolor{red}{$A$ is \textit{triangularisable}} $\Leftrightarrow$ \textcolor{red}{$A$ is similar (Conjugate) to a \textit{upper triangular} matrix $B$.}

\quad \textbf{Corollary II}: {\small Let $f:V\to V$ be End. $V$ is finite-dim. \quad $f$ is \textit{triangularisable} $\Leftrightarrow$ $\exists$ subspaces $V_0=\{0\}\subset V_1\subset\cdots\subset V_n=V$ s.t. $f(V_i)\subseteq V_i$.}

\quad \textcolor{red}{\textbf{Corollary III|nilpotent}: For $A\in Mat(n;F)$. $A$ is \textit{nilpotent} (i.e. $A^k=0$ for some $k$) $\Leftrightarrow$ $\chi_A(x)=x^n$}
...
\fbox{\parbox[c]{0.98\textwidth}{
\textcolor{red}{\textbf{Cayley-Hamilton Theorem}: Let $R$ be \textit{commutative ring}. $A\in Mat(n;R)$. \quad Then: for $\chi_A(x)$ \quad $\chi_A(A)=0$}

对于Jordan 块, 考虑$J-\lambda I\to$ nilpotent. 可以得到一些多项式的性质=0
}}

\fbox{\parbox[c]{0.98\textwidth}{
\np\footnotesize 此框为一些有关前面Ring的性质: by Euclidean algorithm

\textbf{Bézout's identity}: if $gcd(f,g)=1$, then $\exists$ $a,b\in R[x]$ s.t. $af+bg=1$. 

\textbf{About Ideal}: $_{R[X]}\langle f,g\rangle=\gcd(f,g)R[X]$ \quad $_{R[X]}\langle f\rangle + _{R[X]}\langle g\rangle =_{R[X]}\langle \gcd(f,g)\rangle$ \quad $_{R[X]}\langle f\rangle \cap _{R[X]}\langle g\rangle =_{R[X]}\langle \text{lcm}(f,g)\rangle$

\textbf{About Quotient}: {\scriptsize 通过欧几里得算法, 可以寻找到 商空间中的元素的代表.} \quad {\scriptsize e.g. $\mathbb{F}_2[X]/\langle X^2+1\rangle=\{I, 1+I, X+I, X+1+I\}$} {\scriptsize (即所有小于Ideal deg元素|欧几里得算法得到的余数)}

\textbf{Find Units in Quotient}: $g\in R/I$ is a unit iff $\gcd(f,g)=1$ 

\quad (Hint: ($\Rightarrow$) $fg+I=1+I\to fg-1\in I\to fg-1=h\cdot 1 |h\cdot i\to$Euclidean algorithm; $(\Leftarrow)$ Bézout's identity + Euclidean algorithm)
}}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    {\tiny ps: \textit{Cauchy-Schwarz Inequality}, "=" iff $\vec{x},\vec{y}$ are linearly dependent. \quad \textit{Triangle Inequality}, "=" iff $\vec{x},\vec{y}$ are linearly dependent, and they have same direction. (i.e. $\vec{x}=\lambda\vec{y},\lambda\ge0$)}
\end{enumerate}
}}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textbf{Observations}: \textbf{I}. {\small For $\{\vec{v}_1,...,\vec{v}_n\}$ orthonormal basis. \quad \textcolor{red}{$\vec{v}=\sum_{i=1}^n(\vec{v},\vec{v}_i)\vec{v}_i$.}} \qquad \textbf{II}. {\scriptsize For orthonormal Family, 可直接用勾股定理, and 很明显LI}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item Then: \textcolor{red}{\textit{orthogonal projection} $\pi_U:V\to V$ by $\vec{v}\mapsto\sum_{i=1}^m(\vec{v},\vec{e}_i)\vec{e}_i$}
    ...
    \item \textcolor{red}{\textbf{Closest Approximation}}: Since $\|\vec{v}-\vec{u}\|^2=\|\vec{v}-\pi_U(\vec{v})\|^2+\|\pi_U(\vec{v})-\vec{u}\|^2$ \quad $\Rightarrow$ \quad $\vec{u}=\pi_U(\vec{v})$ is the closest vector in $U$ to $\vec{v}$.
\end{enumerate}


\subsection{Basic Properties of Adjoint and Self-Adjoint} % Basic Properties of Adjoint and Self-Adjoint
...
\fbox{\parbox[c]{0.98\textwidth}{
\textbf{IPS isomorphism of $V$}: {\small A linear map $T:V\to W$ is \textit{IPS isomorphism} of $V$ (and $W$) if: \ \ $^1$ $T$ is isomorphism \quad $^2$ $(T\vec{v}_1,T\vec{v}_2)=(\vec{v}_1,\vec{v}_2)$ \ \ {\tiny $\forall \vec{v}_1,\vec{v}_2\in V$}}
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item \textbf{I}. $_{\mathcal{A}}[T^*]_{\mathcal{A}}=\overline{(_{\mathcal{A}}[T]_{\mathcal{A}})}^T$ \qquad \textbf{II}. $(T^*)^*=T$
    \item \textbf{Self-Adjoint}: {\footnotesize $\exists$ orthonormal basis of eigenvectors|Finite $V$} {\scriptsize (Spectral)}\ \ $\Leftrightarrow$ \ \ If $T=T^*$ {\scriptsize (self-adjoint)} \ \ $\Leftrightarrow$ \ \ $_{\mathcal{A}}[T]_{\mathcal{A}}=\overline{(_{\mathcal{A}}[T]_{\mathcal{A}})}^T$ {\scriptsize Hermitian/Symmetric}
    \item $\star$ \textbf{Similar}: If matrix $A=_{\mathcal{A}}[f]_{\mathcal{A}}$ and $B=_{\mathcal{B}}[f]_{\mathcal{B}}$ \quad $\Leftrightarrow$ \quad $B=P^{-1}AP$ and $P$ is is \textit{orthogonal}$_{\mathbb{R}}$ or \textit{unitary}$_{\mathbb{C}}$ matrix.
\end{enumerate}
}}
...
\subsection{Advanced Properties of Adjoint and Self-Adjoint} % Advanced Properties of Adjoint and Self-Adjoint

\newpage

\fbox{\parbox[c]{0.98\textwidth}{
\textbf{Properties of Self-adjoint}: Let $T:V\to V$ be a \textit{self-adjoint} linear map on \textit{inner product space} $V$. Then: {\tiny \color{red}注意:inner product space限制了$F=\mathbb{R}| \mathbb{C}$}

\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textbf{Spectral Theorem}: If $V$ is \textit{finite-dimensional}, then $T$ has \textit{orthonormal basis} of \textit{eigenvectors}. {\scriptsize 存在特征值/向量,且正交为基.}
    \item \textbf{Real}: Every eigenvalues of $T$ are real. \qquad \textbf{Orthogonal|$\lambda$}: Eigenvectors of \textit{distinct eigenvalues} are orthogonal.
    \item \textbf{Orthogonal|$T$}: If $\vec{v} \perp \vec{w}$, and $\vec{v}$ is \textit{eigenvector} of $T$. \quad Then, $T\vec{w}\perp \vec{v}$. {\scriptsize \np also: $\vec{w}\perp T\vec{v}$}
\end{enumerate}

\textbf{Spectral for $\mathbb{R}|\mathbb{C}$ Matrix}: If $A\in\text{Mat}(n,\mathbb{R}|\mathbb{C})$ \textit{symmetric}|\textit{hermitian}. Then $A$ has $n$ \textit{real} eigenvalues $\lambda_1,...,\lambda_n$ {\scriptsize (can be repeated)}.

\hspace{10pt} Moreover, $\exists$ orthogonal|Unitary matrix $P$ \ \ s.t. \ \ $P^{T}AP|\overline{P}^TAP=P^{-1}AP=diag(\lambda_1,...,\lambda_n)$.
}}

\textbf{\underline{Real} Quadratic forms}: $Q(x_1,...,x_n):=\sum_{i=1}^n a_{ii}x_i^2+2\sum_{1\le i<j\le n}a_{ij}x_ix_j=\vec{x}^TA\vec{x}$ \quad {\small where $A$ is \textit{real symmetric} matrix, variables $\vec{x}\in\mathbb{R}^n$}

\vspace{-1pt}
\quad {\scriptsize Can be written as $Q(\vec{x})=(A\vec{x},\vec{x})$ where $(\vec{x},\vec{y})=\vec{x}^T\vec{y}$ is \textit{standard inner product}.}
\quad {\scriptsize \textbf{Corollary}: If $A$ is real symmetric matrix. \ \ $\Rightarrow$ \ \ $A=P^T\Lambda P$ \ \ $\Rightarrow$ \ \ $Q(\vec{x})=\sum_{i=1}^n \lambda_i y_i^2$ where $\vec{y}=P\vec{x}$}

\quad \textbf{Theorem}: $Q(\vec{x})=(A\vec{x},\vec{x})\geq0$ (positive definite) $\Leftrightarrow$ all eigenvalues of $A$ are positive. \qquad {\scriptsize ps: $A$ is real symmetric matrix.}
...
\vspace{-1pt}
\quad {\scriptsize 意思是: $A=P^T\Lambda P$ \ \ $\Rightarrow$ \ \ $Q(\vec{x})=\sum_{i=1}^n \lambda_i y_i^2$ \ \ $\Rightarrow$ \ \ $Q(\vec{x})=1$ 是一个``椭圆'' ellipsoid, ``轴''(e.g.半长轴,半短轴)为$\sqrt{\frac{1}{\lambda_1}},...,\sqrt{\frac{1}{\lambda_n}}$.}


\section{Jordan Normal Form {\scriptsize 默认$F$:algebraically closed}} % Jordan Normal Form
...
{\footnotesize \textbf{Superdiagonal}: $x_{i,i+1}$ \quad {\tiny 对角线上面的元素}}
...
\quad \textbf{Arithmetic Multiplicity}: $\dim E^{\text{gen}}(\lambda_i,\phi)$ \ \ $\geq$ \ \ \textbf{Geometric Multiplicity}: $\dim E(\lambda_i,\phi)$ \quad {\scriptsize ps: If $\chi_{\phi}(x)=\prod_{i=1}^s(x-\lambda_i)^{a_i}$, then $\dim E^{\text{gen}}(\lambda_i,\phi)=a_i$}

\quad {\tiny \np 对于linear map/matrix, $\{0\}\subseteq\ker(f)\subseteq\ker(f^2)\subseteq\cdots\subseteq\ker(f^n)$. If $\ker(f^k)=\ker(f^{k+1})$, then $\ker(f^k)=\ker(f^{k+1})=\cdots=\ker(f^n)$. 由此$E^{\text{gen}}$的$a_i$是一个上界(当等于characteristic对应的),但不一定是最小的.}
...
\quad {\small ps:Jordan form 的形状为一,但里面的顺序不一定要一样. (J is unique up to reordering of the Jordan blocks.)}
...
\textbf{Def of Direct Sum}: {\footnotesize $U_1,...,U_k$ subspaces of $V$. $V=U_1\oplus\cdots\oplus U_k$ \ \ if \ \ $^1V=U_1+U_2+\cdots+U_k$ \quad $^2u_1+u_2+\cdots+u_k=0$ \quad $\Rightarrow$ $u_1=u_2=\cdots=u_k=0$ \ $u_i\in U_i$}

\star \textbf{Useful Tool}: \textcolor{red}{$A\vec{v}=\vec{0}$ for $\vec{v}\ne\vec{0} \Leftrightarrow \det(A)=0$}

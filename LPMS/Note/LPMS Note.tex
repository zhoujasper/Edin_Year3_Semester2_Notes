...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item Exchange $p'$ and $q'$ between $\mathcal{B}$ and $\mathcal{N}$ \ \ $\Rightarrow$ \ \ {\small values of new $x_{B}=\widehat{\mathbf{b}}-\overline{\alpha} \ \widehat{\mathbf{a}}_q$} \qquad Update $\mathcal{B},\mathcal{N},B,N,\widehat{\mathbf{b}},...$ \quad {\scriptsize ps: New $B := B + (a_q - B e_p) e_p^T$}
    \item Go to 3.
\end{enumerate}

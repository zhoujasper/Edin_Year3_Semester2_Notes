...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item \textbf{Functions}: If $f,g$ holomorphic at $z_0$, $z_0$ is a zero of $g$, with order $m$. Then:
    \begin{enumerate}[itemsep=-2pt, topsep=-2pt]
...
\quad\textbf{Corollary|Sequence-zero}: Let $D$ \textit{domain}, distinct $z_n\in D$, $z_n\to z_0\in D$ s.t. $f(z_n)=0,\forall n\in\mathbb{N}$ \ \ $\Rightarrow$ \ \ $f(z)=0,\forall z\in D$.

\quad\textbf{Corollary|Sequence-func}: Let $D$ \textit{domain}, distinct $z_n\in D$, $z_n\to z_0\in D$ s.t. $f(z_n)=g(z_n),\forall n\in\mathbb{N}$ \ \ $\Rightarrow$ \ \ $f(z)=g(z),\forall z\in D$.
